...
for i in range(2):
	...
	phase = dm.Phase(ode_class=CombinedODE,
			     transcription=dm.GaussLobatto(num_segments=500, order=3,compressed=True))
	...
	if i == 0:
		...
		phase.add_control(name='gamma',units='deg',lower=50.0,upper=50.0,fix_initial=False,fix_final=False,targets='gamma',ref=50.0)
	...
	if i == 0:
		axes[0].plot(s,V,label='Active')
		axes[1].plot(s,gamma)
	else:
		axes[0].plot(s,V,label='Fixed (50 deg)')
		axes[1].plot(s,gamma)

axes[1].set_xlabel('Distance along centerline (m)')
axes[0].set_ylabel('Velocity (m/s)')
axes[1].set_ylabel('Wing angle (deg)')
#plt.legend()
...
plt.savefig('velcomparison2.pdf',bbox_inches='tight')
plt.show()
